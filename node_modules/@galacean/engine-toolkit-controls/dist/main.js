...
// Prevent gimbal lock.
const ESP = engine.MathUtil.zeroTolerance;
// Spherical.
class Spherical {
 ...
 */
class FreeControl extends engine.Script {
  constructor(entity) {
    super(entity);
    this.input = void 0;
    ...
    this._tempVec = new engine.Vector3();
  ...
  _rotate(moveDelta) {
    if (moveDelta.x !== 0 || moveDelta.y !== 0) {
      ...
      this._cameraTransform.lookAt(this._tempVec, new engine.Vector3(0, 1, 0));
    }
  }
}
  ...
  static onUpdateDelta(control, outDelta) {
    ...
    switch (this._deltaType) {
      ...
      case DeltaType.Distance:
        ...
        const curDistance = engine.Vector2.distance(pointer1.position, pointer2.position);
        if (this._lastUsefulFrameIndex === frameIndex - 1) {
 ...
 */
class OrbitControl extends engine.Script {
  constructor(...args) {
    super(...args);
    this.canvas = void 0;
    this.input = void 0;
    ...
    this._up = new engine.Vector3(0, 1, 0);
    this._target = new engine.Vector3();
    ...
    this._zoomFrag = 0;
    this._scale = 1;
    this._panOffset = new engine.Vector3();
    this._tempVec3 = new engine.Vector3();
  ...
  _pan(delta) {
    ...
    const targetDistance = engine.Vector3.distance(cameraTransform.position, this.target) * (this.camera.fieldOfView / 2) * (Math.PI / 180);
 ...
 * The camera's track controller, can rotate, zoom, pan, support mouse and touch events.
 */
class OrthoControl extends engine.Script {
  /**
   *  Return Whether to enable rotation, the default is true.
   */
  get enableRotate() {
  ...
  constructor(entity) {
    ...
    this.cameraTransform = void 0;
    /** Target position. */
    this.target = new engine.Vector3();
    /** Up vector */
    this.up = new engine.Vector3(0, 1, 0);
    /** Whether to automatically rotate the camera, the default is false. */
    this.autoRotate = false;
    /** The radian of automatic rotation per second. */
    this.autoRotateSpeed = Math.PI;
...
exports.ControlHandlerType = ControlHandlerType;
